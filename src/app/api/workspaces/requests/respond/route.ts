...
export async function POST(request: Request) {
  const payload = (await request.json()) as { requestId?: string; decision?: "approve" | "reject" };
  ...
  try {
    await approveJoinRequest(db, auth, { requestId: payload.requestId ?? "", approved: payload.decision !== "reject" });
    await emitWorkspaceRefresh(auth.workspaceId);
    await emitSessionRefresh();
    return Response.json({ ok: true });
  } catch (error) {
    return Response.json({ message: error instanceof Error ? error.message : "Unable to process request." }, { status: 400 });
  }
}
