...
Docker Compose 실행 후 서버 PC에서 3002 포트가 열려 있으면,
같은 네트워크의 다른 PC들은 아래처럼 접속하면 된다.

```text
http://<서버PC의 LAN IP>:3002
```
...
```bash
curl http://<서버PC-IP>:3002/api/bootstrap
```
...
```bash
QA_BASE_URL=http://<서버PC-IP>:3002 npm run qa:lan
```
...
QA_BASE_URL=http://<서버PC-IP>:3002 QA_USER_A=admin.kim QA_USER_B=kildong.hong QA_PASSWORD=1234 npm run qa:lan
```
...
```bash
QA_BASE_URL=http://<서버PC-IP>:3002 QA_ACCOUNT_ID=whiteboard.qa QA_PASSWORD=1234 npm run qa:whiteboard
```
...
| `ENTERPRISE_MODE` | enterprise 모드 활성화 여부 (`false`면 비활성) | `false` |
| `MONGODB_LICENSE_ACKNOWLEDGED` | enterprise 모드일 때만 필요한 Mongo 라이선스 검토 확인 | `false` |
...
- [ ] 서버 PC의 방화벽에서 `3002` 포트 허용
- [ ] 같은 망 PC에서 `http://<서버PC-IP>:3002` 접속 가능
- [ ] `docker compose ps`에서 `web`, `mongo` 정상
- [ ] `curl http://<서버PC-IP>:3002/api/bootstrap` 응답 확인
